...
def make_all():
    ...
    while alpha < 0 or alpha > 1:
        alpha = float(input("Ingrese el valor de alpha (entre 0 y 1): ").strip())
    resultado = mezcla_alpha(canales_src_crop, canales_dst, mascara, esquina, alpha=alpha)
    nombre_salida = f"resultado_poisson_alpha_{Path(ruta_src).stem}_{Path(ruta_dst).stem}"
    ruta_salida = os.path.join(carpeta_output, nombre_salida)
    mostrar_y_guardar(resultado, ruta_salida)
    print(f"{nombre_salida} creado.")
    
    resultado = mezcla_poisson(canales_src_crop, canales_dst, mascara, esquina, modo="mixed")
    nombre_salida = f"resultado_poisson_mixed_{Path(ruta_src).stem}_{Path(ruta_dst).stem}"
    ruta_salida = os.path.join(carpeta_output, nombre_salida)
    mostrar_y_guardar(resultado, ruta_salida)
    print(f"{nombre_salida} creado.")

    resultado = mezcla_poisson(canales_src_crop, canales_dst, mascara, esquina, modo="normal")
    nombre_salida = f"resultado_poisson_normal_{Path(ruta_src).stem}_{Path(ruta_dst).stem}"
    ruta_salida = os.path.join(carpeta_output, nombre_salida)
    mostrar_y_guardar(resultado, ruta_salida)
    print(f"{nombre_salida} creado.")
...
def main():
    ...
    while opcion == '':
        ...
        print("1. Aplicar una técnica específica")
        print("2. Aplicar todas las técnicas")
        print("3. Salir")
        opcion = input("Ingresa 1, 2 o 3: ").strip()
        if opcion == '1':
            make_single()
